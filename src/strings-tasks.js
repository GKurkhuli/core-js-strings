...
function isPalindrome(str) {
  ...
  const spaceless = str.replace(/[^\w]/g, '').toLowerCase();
  const rev = spaceless.split('').reverse().join('');
  return rev === spaceless;
}
...
function invertCase(str) {
  ...
  const iverse = letters.map((letter) =>
...
function getCardId(value) {
  ...
  const id = ranks.indexOf(rank) + suits.indexOf(suit) * ranks.length;
  return id;
}
